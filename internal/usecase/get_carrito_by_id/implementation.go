...
func (useCase Implementation) Execute(id uint64) (responses.GetDetalleCarrito, error) {
	...
	for _, dulcesInCarrito := range dulcesIDList {
		dulce, err := useCase.DulcesProvider.GetDetailByID(dulcesInCarrito.DulceID)
		if err != nil {
			return responses.GetDetalleCarrito{}, err
		}
		...
		dulceInCarritoItem := responses.NewDulceInCarrito(dulce, dulcesInCarrito.Unidades, dulcesInCarrito.Subtotal)
